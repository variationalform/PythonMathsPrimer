...
\setbeamertemplate{footline}
         {\quad\copyright\, Simon Shaw (2026)}
...
\subtitle[beamer]{A Crash Course}
\institute[Maths]
          {Mathematics}
...
\pause
The turnover of each of these industries will contain cash inputs from themselves
...
itself, plus \pounds 20,000 (0.5 of \pounds 40,000) from F. 

\item[For F:]  \pounds 40,000 requires \pounds 15,000 (0.3 of \pounds 50,000)
...
Plot $\cos(2\pi x)$ in solid blue and $\exp\big(\sin(4\pi x)\big)$ in dashed red for $x\in[-1,5]$
\end{block}
...
\only<2>{\vspace{-1cm}\begin{center}\includegraphics[width=10cm]{../notebook/gfx/my2Dplot.eps}\end{center}}
...
Plot $\blue 2^{3\sin(3\pi x)}$ in solid dash-dot blue and
$\blue \ln\big(1.2+\sin(3\pi x)\big)$ in dotted red for $\blue x\in[-4,3]$
\end{block}
...
\smallskip
\only<2>{\alert{The names are erased --- can one line identify the person?}}
...
%  - - - - - - - - - - - - - - - - - - - - - - - -      FRAME
\begin{frame}[fragile]\frametitle{Simulation --- Planning the code}
